...
-- ========== [ FAST GOLD FARM (ADDED MODULE) ] ==========
-- Данные маршрута (вставлены из внешнего файла)
...
-- Основные сервисы и локальные переменные
local HttpService = game:GetService("HttpService")
...
-- Контейнер маркеров
local MARKER_FOLDER_NAME = "FastGoldMarkers_v2"

local function ClearFastGoldMarkers()
    local f = workspace:FindFirstChild(MARKER_FOLDER_NAME)
    if f then f:Destroy() end
end

local function CreateMarkers(points)
    ClearFastGoldMarkers()
    ...
    for i,pt in ipairs(points) do
        ...
        p.Position = Vector3.new(pt.X or 0, pt.Y or 0, pt.Z or 0)
        p.Transparency = 0.25
...
-- Парсинг JSON и подготовка точек
local function ParseFastGoldData()
    if not FAST_GOLD_JSON_DATA then
        return nil, "FAST_GOLD_JSON_DATA not found"
    end
    local ok, data = pcall(function() return HttpService:JSONDecode(FAST_GOLD_JSON_DATA) end)
    if not ok or not data or not data.position then
        return nil, "Failed to parse FAST_GOLD_JSON_DATA"
    end
    local waits = data.waits or {}
    local types = data.types or {}
    local positions = data.position or {}
    return { waits = waits, types = types, positions = positions }, nil
end

-- Базовая логика движения: tween до точки с учётом скорости
local FastGoldState = {
    Running = false,
    Speed = FAST_GOLD_FARM_SPEED or 21.5,
    ShowMarkers = FAST_GOLD_FARM_SHOW_MARKERS or false,
...
local function MoveToPosition(pos, speed)
    ...
    local dist = (Root.Position - Vector3.new(pos.X, pos.Y, pos.Z)).Magnitude
    local spd = tonumber(speed) or FastGoldState.Speed or 21.5
    local duration = math.max(0.01, dist / math.max(0.0001, spd))
    local tween = TweenService:Create(Root, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = CFrame.new(pos.X, pos.Y, pos.Z)})
    ...
    while not completed do
        if not FastGoldState.Running then
            tween:Cancel()
            if conn then conn:Disconnect() end
            return false
        end
        RunService.Heartbeat:Wait()
        if tick() - t0 > duration * 3 + 2 then
            break
        end
    end
    return true
...
-- Основной цикл прохода по маршруту
local function StartFastGoldLoop()
    if FastGoldState.Running then return end
    local parsed, err = ParseFastGoldData()
    if not parsed then
        warn("FastGold: parse error:", err)
        return
    end
    FastGoldState.Data = parsed
    if FastGoldState.ShowMarkers and not FastGoldState.MarkersCreated then
        CreateMarkers(parsed.positions)
        FastGoldState.MarkersCreated = true
    end

    FastGoldState.Running = true
    FastGoldState.LoopThread = task.spawn(function()
        local idx = 1
        local total = #parsed.positions
        while FastGoldState.Running do
            if total == 0 then break end
            idx = ((idx - 1) % total) + 1
            local pt = parsed.positions[idx]
            local ptype = parsed.types and parsed.types[idx] or "fly"
            local pwait = tonumber(parsed.waits and parsed.waits[idx]) or 0

            if ptype == "afk" then
                local t = math.max(0.1, pwait or 1)
                local waited = 0
                while waited < t do
                    if not FastGoldState.Running then break end
                    RunService.Heartbeat:Wait()
                    waited = waited + task.wait(0)
                end
            else
                local ok = MoveToPosition(pt, FastGoldState.Speed)
                if not ok then
                    break
                end
                if pwait and pwait > 0 then
                    local waited = 0
                    local t = pwait
                    while waited < t do
                        if not FastGoldState.Running then break end
        ...
        FastGoldState.Running = false
    end)
end

local function StopFastGoldLoop()
    FastGoldState.Running = false
    if FastGoldState.LoopThread then
        FastGoldState.LoopThread = nil
    end
end

-- GUI: интеграция в Tabs.FarmGold (должен существовать)
if Tabs and Tabs.FarmGold then
    local show_markers_toggle = Tabs.FarmGold:CreateToggle("fg_show_markers", {
        Title = "Show Route Markers",
        Default = FastGoldState.ShowMarkers
    })
    show_markers_toggle:OnChanged(function(val)
        FastGoldState.ShowMarkers = val
        if val then
            local parsed, err = ParseFastGoldData()
            if parsed then
                CreateMarkers(parsed.positions)
            ...
            ClearFastGoldMarkers()
            FastGoldState.MarkersCreated = false
        end
    end)

    local speed_slider = Tabs.FarmGold:CreateSlider("fg_speed", {
        Title = "Fly Speed",
        Min = 1,
        Max = 500,
        Default = FastGoldState.Speed,
        Rounding = 1
    })
    speed_slider:OnChanged(function(val)
        FastGoldState.Speed = val
    end)

    Tabs.FarmGold:CreateButton({
        Title = "Load FastGold Route",
        Callback = function()
            local parsed, err = ParseFastGoldData()
            if not parsed then
                warn("FastGold: cannot load route -", err)
                return
            end
            if FastGoldState.ShowMarkers then
                CreateMarkers(parsed.positions)
                FastGoldState.MarkersCreated = true
            end
            print("FastGold: route loaded. Points:", #parsed.positions)
        end
    })

    Tabs.FarmGold:CreateButton({
        Title = "Start FastGold",
        Callback = function()
            if FastGoldState.Running then
                warn("FastGold: already running")
                return
            end
            StartFastGoldLoop()
            print("FastGold: started")
        end
    })
    ...
    Tabs.FarmGold:CreateButton({
        ...
        Callback = function()
            StopFastGoldLoop()
            ClearFastGoldMarkers()
            print("FastGold: stopped")
        end
    })
else
    warn("FastGold integration failed: Tabs.FarmGold not found")
end
